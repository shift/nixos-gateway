...
# NixOS Gateway Test Runner and Log Collector
...
discover_tests() {
    ...
    # Find all .nix test files in tests directory
...
run_test() {
    ...
    # Create test-specific log
    {
        ...
        if command -v nix >/dev/null 2>&1; then
            ...
            if [[ "$test_file" =~ test\.nix$ ]]; then
                # Standard Nix test
                timeout 300 nix-build "$test_file" 2>&1 || exit_code=$?
            else
                # Try as a NixOS configuration test
                timeout 300 nix-instantiate --eval "$test_file" 2>&1 || exit_code=$?
            fi
        else
...
# Main execution
main() {
    log_header "NixOS Gateway Test Runner"
